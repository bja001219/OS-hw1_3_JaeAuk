...
void *MoveVertical(void *vparam)
{
    ...
    while (cont) {
        ...
        while (cont && heady - len + 1 <= height) {
            ...
            if (heady < param->rect->top || x < param->rect->left || x > param->rect->right) {
                if (heady <= height) {
            ...
            if (taily >= 1) {
                pthread_mutex_lock(&mutex);
                if (taily < param->rect->top || taily > param->rect->bottom || x < param->rect->left || x > param->rect->right) {
                    PrintXY(x, taily, " ");
                } else if (x >= param->rect->left && x <= param->rect->right && taily == param->rect->top) {
                    PrintXY(x, taily, "*");
                }
                pthread_mutex_unlock(&mutex);
            }
